				...
				$('#tbody-variants').append('<tr class="tr-variant" id="trVariant'+checkId+'">'+
												'<td>'+
													'<div class="checkbox checkbox-primary">'+
														'<input id="checkboxVariant'+checkId+'" type="checkbox" checked>'+
												...
												'</td>'+
												'<td>'+
													'<input type="number"  id="priceVariant'+checkId+'" class="form-control input-caracter" step="0.01" min="0" max="1000000" required data-parsley-min="0" data-parsley-group="block-variant">'+
												'</td>'+
												'<td>'+
													'<input type="number" id="stockVariant'+checkId+'" class="form-control input-caracter" step="0.01" min="0" max="1000000" required data-parsley-min="0" data-parsley-group="block-variant">'+
												'</td>'+
											'</tr>');
    ...
    ProductComponent.prototype.onDocReady = function(e) {	
		...
		$('#btn-add-extra').click(function() {
			...
			var $valid = $('#extraForm').parsley().validate({group: 'block-1'});
					if(!$valid) {
						...
						var newDiv = '<div class="form-group clearfix" id="newExtraId' + newExtraId + '">'+
										'<div class="col-sm-7">'+
											'<input type="text" class="form-control" readonly value="'+ extraLabel +'" id="extraName' + newExtraId + '"/>'+
										'</div>'+
										'<div class="col-sm-3">'+
											'<input type="number" class="form-control" readonly value="'+ extraPrice +'" id="extraPrice' + newExtraId + '"/>'+
										'</div>'+
										'<div class="col-sm-1">'+
											'<button type="button" class="btn btn-icon waves-effect waves-light btn-danger m-b-5" id="btnId' + newExtraId + '">'+
												'<i class="fa fa-remove" id="removeIconId' + newExtraId + '"></i>'+
											'</button>'+
										'</div>'+
									'</div>';
		...
		$('#btn-add-option').click(function() {
			var newVariant = '<div class="form-group" id="div-option'+newOptionId+'">'+
								'<div class="col-md-3">'+
									'<input type="text" placeholder="Talla" id="option'+newOptionId+'" maxlength="30" class="form-control option" placeholder="Ej. Tamaño" required data-parsley-group="block-option">'+
								'</div>'+
								'<div class="col-md-7">'+
		  ...
		  */
			...
			function continueToList( message ) 
				...
				swal({ title: 'Error al guardar',
				  ...
				  /**
				  *	Function getIndexIfObjWithAttr
				  */
				...
				if( !$validVariants || !$validOptions || !$validProduct ) {	
				 ...
				 // console.log("$validBarcode: " + $validBarcode);			
					...
					$( "tr[class^='tr-variant']" ).each(function( index ) {
					   // console.log(  "Obteniendo variante"  );
					  //variantList.push(this.id.substring(9));
					  
					  var variantId = this.id.substring(9);
					  
					  var actualBarcode = $('#barcodeVariant'+variantId).val();
					  
					   // console.log(  actualBarcode  ); 
					  if( getIndexIfObjWithAttr(variantList, 'barCode', actualBarcode ) == -1 )							  
					  		{							
							var variant = {};
							
//							$("#hasTax").is(':checked');
							
							var chsckId = "#checkboxVariant" + variantId;
//							console.log( "chsckId: ", chsckId);
//							console.log( "chsckId is checked: ", $(chsckId).is(':checked'));
							
							if( $(chsckId).is(':checked') )
								{
					...
					if(!$validProduct)
						{
						continueToList( "Código de barras duplicado en variantes" );
						return false;
						}

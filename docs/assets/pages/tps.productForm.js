...
!function($) {
		...
		function buildVariants() {
			...
			$.each( cart, function( key, value ) {
				...
				$.each( valueOption, function( key2, value2 ) {
					...
					if(value2.length>0) {
						singleCombination += '<button type="button" class="btn btn-success waves-effect waves-light btn-xs m-b-5 btn-mod">'+value2+'</button>';
					}else if(valueOption.length==1&&value2.length==0) {
						$('#datatable-buttons').hide();
					}							
				});
				
				$('#tbody-variants').append('<tr class="tr-variant" id="trVariant'+checkId+'">'+
												'<td>'+
													'<div class="checkbox checkbox-primary">'+
														'<input id="checkboxVariant'+checkId+'" type="checkbox" checked><label for="checkbox'+checkId+'"></label>'+
													'</div>'+
												'</td>'+
												'<td>'+
												...
												'</td>'+
												'<td>'+
													'<input type="text" id="barcodeVariant'+checkId+'" maxlength="30" class="form-control input-caracter" required data-parsley-group="block-variant">'+
												'</td>'+
												'<td>'+
													'<input type="text" id="skuVariant'+checkId+'" maxlength="30" class="form-control input-caracter">'+
												'</td>'+
												'<td>'+
													'<input type="number"  id="priceVariant'+checkId+'" step="0.01" min="0" max="1000000" class="form-control input-caracter" required data-parsley-min="0" data-parsley-group="block-variant">'+
												'</td>'+
												'<td>'+
													'<input type="number" id="stockVariant'+checkId+'" step="0.01" min="0" max="1000000" class="form-control input-caracter" data-parsley-min="0" data-parsley-group="block-variant">'+
												'</td>'+
												'</tr>');
    ...
    ProductComponent.prototype.onDocReady = function(e) {	
		...
		$('#btn-add-option').click(function() {
			var newVariant = '<div class="form-group" id="div-option'+newOptionId+'">'+
								'<div class="col-md-3">'+
									'<input type="text" placeholder="Nombre" id="option'+newOptionId+'" maxlength="30" class="form-control option" placeholder="Ej. Tamaño" required data-parsley-group="block-option">'+
								'</div>'+
								'<div class="col-md-7">'+
									'<input type="text" id="modifier'+newOptionId+'" maxlength="100" class="form-control modifier" data-role="tagsinput" placeholder="Ej. Mediano, Chico, etc"/>'+
								'</div>'+
								'<div class="col-md-2">'+
									'<button type="button" class="btn btn-icon waves-effect waves-light btn-danger m-b-5 visible-options" id="btn-remove-option'+newOptionId+'">'+
										'<i class="fa fa-remove" id="removeOptionIconId' + newOptionId + '"></i>'+
									'</button>'+
								'</div>'+
							'</div>';					
			...
			if( !$('#card-variants').hasClass("card-box-empty") ){
				...
				if( !$validVariants || !$validOptions || !$validProduct ) {	
				 ...
				 // console.log("$validBarcode: " + $validBarcode);			
					...
					var optionList = [];						
					//Recupera ID de cada div-option 
					$( 'td[class^="tags-input"] button[class^="btn"]' ).each(function( index ) {						
						optionList.push(this.innerHTML);
					});
					//console.log( "optionList: " + JSON.stringify(optionList) );
					
					//Recupera ID de cada  trVariant
					$( "tr[class^='variant-to-click']" ).each(function( index ) {
					  //console.log( index + ": " + this.id );
					  //variantList.push(this.id.substring(9));
...
//					  var variantId = this.id.substring(9);
							...
							var variantId = $("#"+this.id).data( "variantid" )
							
							variant.id 	= variantId;
//							variant.price = $('#priceVariant'+variantId).val();
							variant.price = $('#price').val();
//							variant.cost = $('#cost').val();
							variant.barCode = $('#barcode' + variantId).html();
							variant.sku = $('#sku' + variantId).html();
//							variant.stockQuantity = $('#stockVariant'+variantId).val();
							
							var combination = [];
							var optionListId = 0;
							$( '#feature' + variantId + ' button' ).each(function( index, item ) {
								//console.log( "Combination: " + JSON.stringify(this.innerHTML) );
								//console.log( "Data-feature: " + $(item).data('feature') );
//								console.log('#feature' + variantId);
								combination.push({"name":$(this).data('feature'), "value":$(this).html()});
								optionListId++;
							});
...
//				console.log("Price: " + price);
				...
				if (cost > 0 && price > 0 ){
					product.cost = parseFloat( cost ).toFixed(2);
					...
					updateEntity(product, 'productEditForm').done(function(data){
			            ...
			            console.log("responseCode: " + data.response.responseCode);
			            console.log("responseCode: " + data.response.responseMessage);
					...
					//ATENCION!!! aplicar guardar la actualización del producto.
					//continueToList('El costo y/o precio no puede ser 0 o nulo.\nIngresa un precio y costo válido.');
    ...
    UpdateFormUnique.prototype.onDocReady = function(entity, variants) {	
		...
		$('#name').val(entity.name);
				...
				$( variants ).each(function( index ) {	
					...
					$('#sku').val(this.sku == "null" ? "" : this.sku );
		 ...
		 // console.log("variants: "+  JSON.stringify( variants ) );
				...
				$( variants ).each(function( index ) {	
					...
					$.each( this.featureList, function( index2 ) {
						singleCombination += '<button type="button" class="btn btn-success waves-effect waves-light btn-xs m-b-5 btn-mod" data-feature="'+this.name+'">'+this.value+'</button>';
					});

					var thisSku = this.sku == "null" ? "" : this.sku;
					
					var newVariant = '<tr style="cursor: pointer;" id="trVariant' +checkId+ '" data-entityid="'+ entity.id + '" data-variantid="' + this.id +'" class="variant-to-click">'
										+ '<td id="feature' + this.id + '" class="tags-input">' + singleCombination + '</td>'
										+ '<td id="barcode' + this.id +'">' + this.barCode + '</td>'
										+ '<td id="sku' + this.id + '">' + thisSku + '</td>'
										+ '<td id="cost' + this.id + '"> $ ' + parseFloat( this.inventory.cost ).toFixed(2) + '</td>'
										+ '<td id="price' + this.id + '"> $ ' + parseFloat( this.inventory.priceList[0].value ).toFixed(2) + '</td>'
								    + '</tr>';
